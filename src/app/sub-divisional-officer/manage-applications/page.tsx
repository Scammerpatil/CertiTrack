...
export default function ApplicationsPage() {
  ...
  const fetchApplications = async () => {
    const response = await fetch("/api/sub-divisional-officer/applications");
  ...
  const handleStatusChange = async (
    ...
    const response = axios.patch("/api/certificate/update-application", {
      remark,
      status: status ? "Approved by SDO" : "Rejected By SDO",
      appId: appId,
      officer: "SDO",
              ...
              {filteredApplications.length === 0 ? (
                ...
                filteredApplications.map((app, index) => (
                  <tr key={app._id}>
                    ...
                    <td>
                      <Link
                        href={`/sub-divisional-officer/view-details?id=${app._id}&type=${app.type}`}
                      >
                        {app._id}
                      </Link>
                    </td>
                    ...
                    <th className="space-x-3">
                      ...
                      ) : app.status === "Verified by CO" ||
                        app.status === "Approved by SDO" ||
                        app.status === "Rejected By SDO" ||
                        app.status === "Certificate Issued" ? (
                        app.remarkBySDO ? (
                          <span>{app.remarkBySDO}</span>
                        ) : (
                          <>
                            <input
                              type="text"

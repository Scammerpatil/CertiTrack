...
export async function POST(req: NextRequest) {
  const { nonCreamyLayerCertificate } = await req.json();
  const user = await User.findById(nonCreamyLayerCertificate.applicantId);
  if (!user) {
    return NextResponse.json({ message: "User not found" }, { status: 404 });
  }
  try {
    if (nonCreamyLayerCertificate) {
      const newNonCreamyLayerCertificate = new NonCreamyLayer(
        nonCreamyLayerCertificate
      );
      const certificate = await newNonCreamyLayerCertificate.save();

      // Make new Notification and send email
      const notification = new Notification({
        recipientModel: "User",
        recipient: nonCreamyLayerCertificate.applicantId,
        type: "CERTIFICATE_APPLIED",
        title: "Non-Creamy Layer Certificate Applied",
        message:
          "Your non-creamy layer certificate application has been submitted.",
        relatedCertificateModel: "NonCreamyLayer",
      ...
      const certificateOfficer = await CertificateOfficer.findOne({
        taluka: nonCreamyLayerCertificate.generalInfo.taluka,
        district: nonCreamyLayerCertificate.generalInfo.district,
      });
      if (certificateOfficer) {
        const notificationToCO = new Notification({
          ...
          type: "CERTIFICATE_APPLIED",
          title: "New Non-Creamy Layer Certificate Application",
          message: `A new non-creamy layer certificate application has been submitted by ${nonCreamyLayerCertificate.applicantId}.`,
          relatedCertificateModel: "NonCreamyLayer",
          relatedCertificate: certificate._id,
          sendAt: new Date(),
        });

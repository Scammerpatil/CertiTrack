...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch application
    const application = await BirthCertificate.findById(id);
    if (!application) {
      return NextResponse.json(
    ...
    doc.render({
      ...
      motherName: application.parents.mother,
      fatherName: application.parents.father,
      hospitalName: application.hospitalDetails.name,
      hospitalAddress: application.hospitalDetails.address,
      hospitalContact: application.hospitalDetails.contactNumber,
